...
class Cipher:
    ...
    def decode_caesar(self, cipher_text, n):
        ...
        for char in cipher_text.lower():
            if char not in self.letter_dict.keys():
                decode += char
                continue
    ...
    def score_string(self, string):
        ...
        for char in string.upper():
            if char in self.letter_heuristics:
                self.letter_heuristics[char] += 1
        return self.letter_heuristics

    def crack_caesar(self, cipher_text):
        for key in range(27):
            translated = ''
            for char in cipher_text.lower():
                if char in self.letter_dict.keys():
                    num = self.inverse_dict[char]
                    num = num - key
                    if num < 0:
                        num = num + 27
    ...
    def encode_otp(self, message):
        ...
        messageToBinary = string_to_binary(message)
...
if __name__ == '__main__':
    ...
    cipher_suite = Cipher()
    # # print("---------------------PART 1: CAESAR-----------------------")
    # message = "my name is kokil dhakal i live in usa"
    #
